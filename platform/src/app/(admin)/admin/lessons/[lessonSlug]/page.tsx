...
/**
 * Determines whether a student has completed a specific task.
 * Uses the mock data's per-lesson completed counts to simulate
 * per-task completion (marks the first N tasks in order as done).
 */
function hasStudentCompletedTask(
  ...
  const student = MOCK_STUDENTS.find(
    (s) => s.user.githubNickname === studentNickname
  );
...
export default function LessonCompletionPage({
  ...
  // Build ordered lists per category for index-based completion lookups
  ...
  return (
    ...
    >
      ...
      <p className="mb-6 text-sm text-gray-500">
        {allTasks.length} homework task{allTasks.length !== 1 ? "s" : ""} &middot;{" "}
        {MOCK_STUDENTS.length} student{MOCK_STUDENTS.length !== 1 ? "s" : ""}
      </p>
